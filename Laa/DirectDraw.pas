//Nota: Este archivo es equivalente a un ".h" de c para utilizar los dlls de direct draw.
unit DirectDraw;
...
// Delphi 6 can't handle interface in variant records
// so we have to use pointers instead (which can be type-casted into interfaces):
// use VER130 for delphi5, VER140 is for delphi6
{$IFDEF VER140}
...
begin
  case Value of
    DD_OK:result:='';
    DDERR_INVALIDMODE:result:='Tu tarjeta de video no soporta el modo requerido'
    else
    begin
      str(value,result);
      result:='Error DirectDraw #'+result+', necesitas DirectX7 o superior';
    end;
  end;
{  case Value of
    DD_OK: Result := 'El requerimiento fué completado exitosamente';
    DDERR_ALREADYINITIALIZED: Result := 'Este objeto ya está inicializado';
    DDERR_BLTFASTCANTCLIP: Result := 'Una superficie adjuntada a un objeto clip no puede llamar a bltfast';
    DDERR_CANNOTATTACHSURFACE: Result := 'Esta superficie no puede ser adjuntada a la superficie indicada';
    DDERR_CANNOTDETACHSURFACE: Result := 'Esta superficie no puede ser desadjuntada a la superficie indicada';
    DDERR_CANTCREATEDC: Result := 'Windows no puede crear más DCs';
    DDERR_CANTDUPLICATE: Result := 'No puedo duplicar superficies primarias y 3D, o superficies que son creadas implícitamente';
    DDERR_CLIPPERISUSINGHWND: Result := 'Se intentó dar una lista de clips a un objeto clipper que ya está monitoreando un hwnd';
    DDERR_COLORKEYNOTSET: Result := 'No se especificó el color clave src para esta operación';
    DDERR_CURRENTLYNOTAVAIL: Result := 'No está disponible por el momento';
    DDERR_DIRECTDRAWALREADYCREATED: Result := 'Un objeto DirectDraw representando este driver ya fue creado para este proceso';
    DDERR_EXCEPTION: Result := 'Excepción encontrada al realizar la operación requerida';
    DDERR_EXCLUSIVEMODEALREADYSET: Result := 'Se intentó poner en modo Cooperativo estando ya en Exclusivo';
    DDERR_GENERIC: Result := 'Fallo general';
    DDERR_HEIGHTALIGN: Result := 'El alto del rectángulo proveido no es múltiplo de alineación reqd';
    DDERR_HWNDALREADYSET: Result := 'El nivel cooperativo HWND ya esta activado. No puede recomenzar si el proceso tiene superficies o paletas creadas';
    DDERR_HWNDSUBCLASSED: Result := 'El HWND usado por el nivel cooperativo ha sido subclasificado para evitar que DirectDraw restaure su estado';
    DDERR_IMPLICITLYCREATED: Result := 'Esta superficie no puede restaurarse por ser creada implicitamente';
    DDERR_INCOMPATIBLEPRIMARY: Result := 'No puede concordar la superficie primaria creada con la superficie primaria existente';
    DDERR_INVALIDCAPS: Result := 'Uno o más bits de caps. pasados al retorno de llamada son incorrectos';
    DDERR_INVALIDCLIPLIST: Result := 'DirectDraw no puede trabajar con la lista de clips dado';
    DDERR_INVALIDDIRECTDRAWGUID: Result := 'El GUID dado a DirectDrawCreate no es un identificador de driver DirectDraw';
    DDERR_INVALIDMODE: Result := 'DirectDraw no soporta el modo requerido';
    DDERR_INVALIDOBJECT: Result := 'DirectDraw recibió un puntero que no era un objeto DIRECTDRAW (o no era válido)';
    DDERR_INVALIDPARAMS: Result := 'Uno o más parámetros son inadecuados';
    DDERR_INVALIDPIXELFORMAT: Result := 'LA especificación del formato de pixel no es válida';
    DDERR_INVALIDPOSITION: Result := 'Retorno cuando la posición de la cubierta(overlay) del destino ya no es adecuada para el destino';
    DDERR_INVALIDRECT: Result := 'El Rectángulo indicado no es válido';
    DDERR_LOCKEDSURFACES: Result := 'No se puede realizar la operación porque una o más superficies están aseguradas';
    DDERR_NO3D: Result := '3D ausente';
    DDERR_NOALPHAHW: Result := 'No hay aceleración alpha disponible o presente para realizar la operación';
    DDERR_NOBLTHW: Result := 'Hardware para blitter está ausente';
    DDERR_NOCLIPLIST: Result := 'Sin lista de clips';
    DDERR_NOCLIPPERATTACHED: Result := 'Sin objeto clipper adjuntado al objeto superficie';
    DDERR_NOCOLORCONVHW: Result := 'sin aceleración de conversión de color. No puede realizarse la operación';
    DDERR_NOCOLORKEY: Result := 'La superficie no tiene clave de color';
    DDERR_NOCOLORKEYHW: Result := 'No se puede realizar la operación por no existir soporte de hardware para la clave de color destino';
    DDERR_NOCOOPERATIVELEVELSET: Result := 'Función de creación llamada sin que se ejecute el metodo SetCooperativeLevel de un objeto DirectDraw';
    DDERR_NODC: Result := 'No se creó un DC para esta superficie';
    DDERR_NODDROPSHW: Result := 'Sin hardware ROP DirectDraw';
    DDERR_NODIRECTDRAWHW: Result := 'Un objeto DirectDraw sólo para hardware se creo sin que el driver soporte algún hardware';
    DDERR_NOEMULATION: Result := 'No está disponible la emulación por software';
    DDERR_NOEXCLUSIVEMODE: Result := 'La operación requiere el modo exclusivo, pero la aplicación no la tiene';
    DDERR_NOFLIPHW: Result := 'Intercambio de superficies visible no está soportada';
    DDERR_NOGDI: Result := 'Sin GDI';
    DDERR_NOHWND: Result := 'La notificación al Clipper necesita un HWND o no se a indicado previamente un HWND como el HWND del modo Cooperativo';
    DDERR_NOMIRRORHW: Result := 'Sin hardware disponible';
    DDERR_NOOVERLAYDEST: Result := 'GetOverlayPosition fue llamado en un overlay que nunca llamo a UpdateOverlay para establecer un destino';
    DDERR_NOOVERLAYHW: Result := 'No se puede realizar por no existir o estar disponible hardware para overlay';
    DDERR_NOPALETTEATTACHED: Result := 'Sin objeto paleta adjuntado a esta superficie';
    DDERR_NOPALETTEHW: Result := 'Sin hardware para soportar paleta de 16 o 256 colores';
    DDERR_NORASTEROPHW: Result := 'La operación no se pudo realiza por que no hay la apropiada op de hardwate raster presente o disponible';
    DDERR_NOROTATIONHW: Result := 'Sin hardware de rotación';
    DDERR_NOSTRETCHHW: Result := 'Sin hardware para ajuste de imagen';
    DDERR_NOT4BITCOLOR: Result := 'DirectDrawSurface no está en paleta de 4 bits de color y se necesita que esté';
    DDERR_NOT4BITCOLORINDEX: Result := 'DirectDrawSurface no está en paleta indexada de 4 bit de color y esto es necesario';
    DDERR_NOT8BITCOLOR: Result := 'DirectDrawSurface no estña en modo de 8 bit de color y esto es necesario';
    DDERR_NOTAOVERLAYSURFACE: Result := 'Un miembro de un overlay fue llamado para una superficie no overlay';
    DDERR_NOTEXTUREHW: Result := 'Sin hardware de mapeo de textura';
    DDERR_NOTFLIPPABLE: Result := 'Se intento hacer flip con una superficie sin esa capacidad';
    DDERR_NOTFOUND: Result := 'El item requerido no se encontró';
    DDERR_NOTLOCKED: Result := 'La superficie no estaba asegurada antes ni con este proceso y se intentó desasegurarla';
    DDERR_NOTPALETTIZED: Result := 'La superficie usada no es una superficie basada en palera';
    DDERR_NOVSYNCHW: Result := 'Sin hardware para sincronizar operaciones de refrescamiento vertical';
    DDERR_NOZBUFFERHW: Result := 'Sin hardwate para zbuffer blitting';
    DDERR_NOZOVERLAYHW: Result := 'Las superficies Overlay no pueden ser z layered basadas en su BltOrder porque el hardwate no soporta z layering de overlays';
    DDERR_OUTOFCAPS: Result := 'El hardware necesitado para la operación ya está ocupado';
    DDERR_OUTOFMEMORY: Result := 'DirectDraw no tiene más memoria para realizar esta operación';
    DDERR_OUTOFVIDEOMEMORY: Result := 'DirectDraw no tiene más memoria de video para esta operación';
    DDERR_OVERLAYCANTCLIP: Result := 'El hardware no soporta overlays clipped';
    DDERR_OVERLAYCOLORKEYONLYONEACTIVE: Result := 'Sólo puede estar activa una clave de color a la vez para overlays';
    DDERR_OVERLAYNOTVISIBLE: Result := 'GetOverlayPosition fue llamado para un overlay oculto';
    DDERR_PALETTEBUSY: Result := 'No se puede acceder a esta paleta mientras la paleta ya está asegurada por otro hilo';
    DDERR_PRIMARYSURFACEALREADYEXISTS: Result := 'Este proceso ya tiene superficie primaria';
    DDERR_REGIONTOOSMALL: Result := 'La Region pasada a Clipper::GetClipList es muy pequeña';
    DDERR_SURFACEALREADYATTACHED: Result := 'Esta superficie ya está adjuntada a esa superficie a la que se quiere adjuntar';
    DDERR_SURFACEALREADYDEPENDENT: Result := 'Esta superficie ya es una dependencia de esa superficie';
    DDERR_SURFACEBUSY: Result := 'No se puede acceder a esta superficie por que la misma ya está asegurada por otro hilo';
    DDERR_SURFACEISOBSCURED: Result := 'No se puede acceder a esta superficie por estar obscurecida';
    DDERR_SURFACELOST: Result := 'El acceso a esta superficie esta siendo rechazado por que la memoria de la superficie se perdió. El objeto DirectDrawSurface representando esta superficie debe haber llamado a Restore';
    DDERR_SURFACENOTATTACHED: Result := 'La superficie requerida no está adjuntada';
    DDERR_TOOBIGHEIGHT: Result := 'El alto pedido es demasiado';
    DDERR_TOOBIGSIZE: Result := 'El tamaño pedido es demasiado, pero el ancho y alto individual están bien';
    DDERR_TOOBIGWIDTH: Result := 'El ancho pedido es demasiado';
    DDERR_UNSUPPORTED: Result := 'Acción no soportada';
    DDERR_UNSUPPORTEDFORMAT: Result := 'El formato FOURCC requerido no es soportado por DirectDraw';
    DDERR_UNSUPPORTEDMASK: Result := 'La Bitmask en el formato de pixel requerido no está soportado por DirectDraw';
    DDERR_VERTICALBLANKINPROGRESS: Result := 'Un refrescamiento vertical está en progreso';
    DDERR_WASSTILLDRAWING: Result := 'DirectDraw indica que el Blt previo que se estaba transfiriendo de o a esta superficie está incompleto';
    DDERR_WRONGMODE: Result := 'Esta superficie no puede ser restaurada por que fue creada en modo diferente';
    DDERR_XALIGN: Result := 'El rectangulo proveido no estaba alineado horizontalmente a los límites requeridos';
    // new:
    DDERR_OVERLAPPINGRECTS: Result := 'No se puede por que los rectángulos destino y origen están solapados en la misma superficie';
    DDERR_INVALIDSTREAM: Result := 'El flujo indicado contiene datos inválidos';
    DDERR_UNSUPPORTEDMODE: Result := 'La pantalla está en modo no soportado!!';
    DDERR_NOMIPMAPHW: Result := 'Sin hardware de mapeo de textura mip-map';
    DDERR_INVALIDSURFACETYPE: Result := 'La superficie es del tipo incorrecto';
    DDERR_NOOPTIMIZEHW: Result := 'El dispositivo no soporta superficies optimizadas, ni de memoria de video';
    DDERR_NOTLOADED: Result := 'La superficie es una optimizada, pero no reservó memoria alguna';
    DDERR_NOFOCUSWINDOW: Result := 'Se intentó crear o colocar un dispositivo ventana sin primero indicar la ventana focalizada';
    DDERR_DCALREADYCREATED: Result := 'Un DC se devolvió para esta superficie y sólo un DC puede ser obtenido por superficie';
    DDERR_NONONLOCALVIDMEM: Result := 'Se intentó reservar memoria de video no local por un dispositivo que no soporta esta operación';
    DDERR_CANTPAGELOCK: Result := 'El intento de asegurar página superficie falló';
    DDERR_CANTPAGEUNLOCK: Result := 'El intento de desasegurar página superficie falló';
    DDERR_NOTPAGELOCKED: Result := 'Se intendo desasegurar una page sin estar asegurada';
    DDERR_MOREDATA: Result := 'El buffer está lleno y hay más datos!';
    DDERR_EXPIRED: Result := 'Los datos expiraron y ya no son válidos';
    DDERR_VIDEONOTACTIVE: Result := 'El puerto de video no está activo';
    DDERR_DEVICEDOESNTOWNSURFACE: Result := 'Las superficies creadas por un dispositivo directDraw no pueden ser usadas directamente por otro dispositivo DirectDraw';
    DDERR_NOTINITIALIZED: Result := 'Se intentó invocar un miembro de interfaz de un objeto DirectDraw creado por CoCreateInstance() sin ser anter inicializado';
    else
    begin
     str(value,result);
     Result := 'Error DD #'+result;
    end
  end;}
end;
