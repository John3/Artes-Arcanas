//Nota: Este archivo es equivalente a un ".h" de c para utilizar los dlls de direct sound
unit DirectSound;
{$MINENUMSIZE 4}
{$ALIGN ON}
...
function DSErrorString(Value: HResult) : string;
begin
  if Value=DS_OK then result:='OK' else
  begin
    str(value,result);
    result:='Error DirecSound #'+result;
  end;
//Comentar para reducir el tamaño del ejecutable
{  case Value of
