...
uses
  ...
  stdctrls, ExtCtrls, WinProcs, MidiOut;
const
//errores
  emidi_Ok=0;
  emidi_ChunkType_Desconocido=10001;
  emidi_ErrorEnEncabezado=10002;
  emidi_Formato_Desconocido=10003;
  emidi_DatoEventoDesconocido=10004;
  emidi_eventoDesconocido=10005;
...
type
  ...
  public
    ...
    OnTrackReady: TEvent;
    constructor Create(aMidiFile:TMidiFile);
  ...
  private
    ...
    fDefaultMidiOutput: TMidiOutput;
  ...
  protected
    ...
    function ProcessHeaderChunk:integer;
    function ProcessTrackChunk:integer;
  ...
  public
    ...
    function  GetTrackLength:integer;
    function  Ready: boolean;
    function ErrorToStr(ErrorCode:integer):string;

    procedure HandleEvent(event: PMidiEvent);
  ...
  published
    ...
    property DefaultMidiOutput: TMidiOutput write FDefaultMidiOutput;
...
constructor TMidiTrack.Create(aMidiFile:TmidiFile);
begin
  inherited Create;
  fMidiFile:=aMidiFile;
  events := TList.Create;
...
begin
  ...
  begin
    if fMidiFile<>nil then
      fMidiFile.HandleEvent(PMidiEvent(events[currentPos]));
...
var
  i: integer;
begin
  if (chunkData <> nil) then FreeMem(chunkData);
...
begin
  if playing then
  begin
    PlayToTime(GetTickCount - LongWord(PlayStartTime));
...
begin
  PlayStartTime := GetTickCount - longWord(currentTime);
  playing := true;
...
begin
  ...
  begin
    us_per_quarter := C60_Millones div val;
    FBpm := C60_Millones div us_per_quarter;
    FusPerTick := us_per_quarter / deltaTicks;
  end;
end;

function TMidifile.ReadChunkHeader:integer;
var
...
begin
  ...
  if chunkType=illegal then result:=emidi_ErrorEnEncabezado else result:=emidi_Ok;
end;
...
function TMidifile.ReadChunk:integer;
begin
  result:=ReadChunkHeader;
  if result<>emidi_Ok then exit;
  ReadChunkContent;
  case chunkType of
    header:
      result:=ProcessHeaderChunk;
    track:
      result:=ProcessTrackCHunk;
    else
      result:=emidi_ChunkType_Desconocido
  end;
end;

function TMidifile.ProcessHeaderChunk:integer;
begin
  result:=emidi_Ok;
  chunkIndex := chunkData;
  inc(chunkIndex);
  ...
  begin
    case chunkIndex^ of
      ...
      2: FfileFormat := multi_asynch;
        ...
        result:=emidi_Formato_Desconocido;
    end;
    inc(chunkIndex);
...
function TMidifile.ProcessTrackChunk:integer;
var
  dTime: integer;
  event: integer;
  len: integer;
...
//  inc(chunkIndex);
  ...
  begin
    currentTrack := TMidiTrack.Create(Self);
...
{        case chunkIndex^ of
        ...
        begin
          ...
          case midiEvent.data1 of
            $51://set tempo
              begin
                us_per_quarter :=(integer(byte(midiEvent.str[1])) shl 16 +
...
//         inc(chunkIndex);
        case event{un byte} of
          ...
          result:=emidi_eventoDesconocido+event shl 16;
           // error
        end;
        currentTrack.putEvent(midiEvent);
      end;
...
begin
  ...
  for i := 1 to l do
  begin
    result:=result+Chr(chunkIndex^);
    inc(chunkIndex);
  end;
end;


function TMidifile.ReadFile:integer;
var
  i,resultadoIO: integer;
begin
  result:=emidi_Ok;
  StopPlaying;
  for i := 0 to Tracks.Count - 1 do
  ...
  FileMode := 0;
  Reset(midiFile);
  while (not eof(midiFile)) and (result=emidi_Ok) do
    result:=ReadChunk;
  CloseFile(midiFile);
  {+$I}
  resultadoIO:=IORESULT;
  if result=0 then result:=resultadoIO;
...
procedure TMidiFile.HandleEvent(event: PMidiEvent);
var us_per_quarter:integer;
    mEvent : TMyMidiEvent;
begin
  if (event.event = $FF) then
  begin
    if (event.data1=$51) then//tempo
    begin
      us_per_quarter :=(integer(byte(event.str[1])) shl 16 +
        integer(byte(event.str[2])) shl 8 +
        integer(byte(event.str[3])));

      //Writeln('Event - Tempo: ',us_per_quarter,' time:',now());
...
function TMidiFile.ErrorToStr(ErrorCode:integer):string;
begin
  case (ErrorCode and $FFFF) of
    2,103:result:='Archivo inexistente';
    emidi_ChunkType_Desconocido:result:='Tipo de bloque desconocido';
    emidi_Formato_Desconocido:result:='Formato desconocido';
    emidi_ErrorEnEncabezado:result:='Error en encabezado midi';

    emidi_DatoEventoDesconocido:result:='Dato de evento desconocido';
    emidi_EventoDesconocido:result:='Evento desconocido, datos:'+inttostr(ErrorCode shr 16);

    emidi_Ok:result:='';
  else
    result:='Error #'+inttostr(ErrorCode and $FFFF)+' Data: '+inttostr(ErrorCode shr 16);
  end;
end;
