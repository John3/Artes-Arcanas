...
const
  ...
  clMidiScopeFont=clWhite;
  clMidiScopeBars=clLime;
  clMidiScopeLines=clYellow;
...
procedure TMidiScope.Clear;
begin
  InicializarValores;
  repaint;
end;

procedure TMidiScope.InicializarValores;
var
 i,j : integer;
begin
  for i := 0 to 15 do
...
begin
  x := pos * BarWidthInc + 2;
  y := 2 + ch * BarHeightInc;
  bitblt(canvas.handle,x, y, BarWidthInc, BarHeightInc,0,0,0,CMBLACKNESS);
  Canvas.Brush.Color := clMidiScopeBars;
  Canvas.FillRect(rect(x, y + (BarHeight - (val div HeightDiv )), x + BarWidth, y + BarHeight))
end;
...
x : integer;
begin
  bitblt(canvas.handle,0,0,width,height,0,0,0,CMBLACKNESS);
  Canvas.Pen.Color := clMidiScopeLines;
  x := 128*BarWidthInc+2;
  Canvas.MoveTo(x,0);
  Canvas.LineTo(x,Height);
  x :=  148*BarWIdthInc+2;
  canvas.Font.Color := clMidiScopeFont;
  for i := 0 to 15 do
  begin
