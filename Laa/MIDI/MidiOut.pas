...
begin
  if FState = mosOpen then
    raise EmidioutputError.Create('No es posible cambiar de dispositivo midi mientras este abierto.')
  else
    if (DeviceID >= midioutGetNumDevs) and (DeviceID <> MIDI_MAPPER) then
      raise EmidioutputError.Create('No existe dispositivo midi #'+Inttostr(DeviceID))
    else
    begin
      FDeviceID := DeviceID;
